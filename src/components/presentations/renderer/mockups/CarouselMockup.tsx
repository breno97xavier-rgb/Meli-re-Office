...
export const CarouselMockup: React.FC<CarouselMockupProps> = ({
  item,
  client,
  signedUrls,
  ...
  return (
    ...
    >
      {slides.length > 0 && currentUrl ? (
        <div className="relative w-full h-full group select-none bg-[#111111] flex items-center justify-center">
          {/* Active Image (4:5 frame with object-contain to prevent any distortion) */}
          <img
          ...
          <div className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-black/65 backdrop-blur-xs text-white text-[11px] font-bold tracking-wider z-10 flex items-center gap-1.5 shadow-md">
          ...
          {currentIndex > 0 && (
            <button
              ...
              className="absolute left-2.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/60 hover:bg-black/85 text-white flex items-center justify-center transition-all backdrop-blur-xs z-10 shadow-lg cursor-pointer hover:scale-105 active:scale-95"
            >
          ...
          {currentIndex < slides.length - 1 && (
            <button
              ...
              className="absolute right-2.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/60 hover:bg-black/85 text-white flex items-center justify-center transition-all backdrop-blur-xs z-10 shadow-lg cursor-pointer hover:scale-105 active:scale-95"
            >
          ...
          {/* Bullets indicator */}
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/40 backdrop-blur-xs z-10">
            {slides.map((_, idx) => (
              <button
                key={idx}
                type="button"
        ...
        <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center bg-[#1A1A1A] text-[#9E9EA0]">
          ...
          <p className="text-xs font-semibold text-white">Lâminas em Produção</p>
          <p className="text-[11px] text-[#7E7E80] mt-1 max-w-[220px]">
