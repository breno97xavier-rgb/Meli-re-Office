...
interface InstagramMockupFrameProps {
  client?: Client | null;
...
export const InstagramMockupFrame: React.FC<InstagramMockupFrameProps> = ({
  ...
  return (
    <div className="w-full max-w-[420px] mx-auto bg-white rounded-2xl border border-[#E8E9EA] shadow-xl overflow-hidden flex flex-col select-none transition-all">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1">
              <span className="text-xs font-bold text-[#1D1D1D] truncate leading-none">
                {handle || 'cliente'}
              </span>
              <span className="w-1.5 h-1.5 rounded-full bg-[#F15A3C]" />
            </div>
            <span className="text-[10px] text-[#9E9EA0] truncate block mt-0.5">
              Publicação Oficial
            </span>
          </div>
        </div>
      ...
      <div className={`relative w-full ${aspectClass} bg-[#111111] overflow-hidden flex items-center justify-center`}>
        {children}
      </div>
        ...
        <div className="p-3 bg-white space-y-2">
          ...
          {captionPreview && (
            <div className="text-xs text-[#1D1D1D] leading-snug">
              ...
              <span className="text-[#444446] line-clamp-2">{captionPreview}</span>
            </div>
          )}
        </div>
      )}
